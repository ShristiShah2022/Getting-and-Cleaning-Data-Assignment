...
## below set of instructions is used to download dataset from UCI Machine Learning Repository

vector<-"https://d396qusza40orc.cloudfront.net/getdata%2Fprojectfiles%2FUCI%20HAR%20Dataset.zip"
download.file(vector,"Dataset.zip",mode = "wb") ## wb mode is to read the binary data of the provided dataset
unzip(zipfile="Dataset.zip",exdir="./data")
path<-file.path("./data","UCI HAR Dataset")
files<-list.files(path,recursive = TRUE) ## following command lists all files present

## below set of instructions is used to read train data
...
## below set of instructions is used to read test data
...
## below command is used to read features

features = read.table(file.path(path, "features.txt"),header = FALSE)

## below command is used to read labels
...
mean_std_subset <- test_plus_train[ , mean_std == TRUE]
